...
def ui_add_student(students, undo_operations, student_id, name, grade):
    try:
        student = create_student(int(student_id), name, int(grade))
        validate_student(student)
        add_student(students, student)
        register_undoable_operation(undo_operations, add_student, delete_student, students, int(student_id))
    except ValueError as ve:
        print("invalid input.", ve)
        raise ve        


def ui_print_student(student, undo_operations=None):
    print("(student_id={0}, name={1}, grade={2})".format(
        student["student_id"], student["name"], student["grade"]), end="\t")
     
 
def ui_print_all(students, undo_operations=None):
    if len(students) == 0:
...
def ui_delete_student(students, undo_operations, student_id):
    delete_student(students, int(student_id))  # TODO data validation
        

def ui_filter(students, undo_operations, grade):
    ui_print_all(filter_students(students, int(grade)))  # TODO data validation


def ui_find_max_grade_student(students, undo_operations):
    ui_print_student(find_max_grade_student(students))
    print()
    
def find_students_by_name(students, undo_operations, t):
...
def register_undoable_operation(undo_operations, caller, handler, *args):
    undo_operations.append([caller, handler, args])
    
def undo(students, undo_operations):
    operation = undo_operations.pop()
...
def run_app():
    ...
    cmds = {"add":ui_add_student, "show":ui_print_all, "delete":ui_delete_student,
            ...
            "find_by_name":find_students_by_name, "undo":undo}
    undo_operations = []
    while True:
        (cmd, args) = read_cmd()
        if cmd == "exit":
            break
        try:
            cmds[cmd](students, undo_operations, *args)
