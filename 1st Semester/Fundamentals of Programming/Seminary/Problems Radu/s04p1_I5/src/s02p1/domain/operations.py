...
#==========================================================================================================
def remove_smaller_than(students, grade):
    students[:] = list(filter(lambda student: get_grade(student) >= grade, students))
    
def sort_students_by_grade(students):
    return sorted(students, key=get_grade, reverse=True)

def find_top_students(students, nr):
    result = sort_students_by_grade(students)
    return result[:nr]

def compute_avg_grade(students, grade):
    result = filter_students(students, grade)
    sum_of_grades = reduce(lambda x, y: x + get_grade(y), result, 0)
    return sum_of_grades / len(result)

def sort_students_by_grade_and_by_name(students):
    return sorted(students, key=lambda x: (get_grade(x), get_name(x)))
