...
class MovieController(object):
    ...
    def addMovie(self,movieId,title,genre,description):
        ...
        redo = FunctionCall(self.addMovie,movieId,title,genre,description)
        undo = FunctionCall(self.deleteMovie,movieId)
    ...
    def deleteMovie(self,movieId):
        ...
        redo = FunctionCall(self.deleteMovie, movieId)
        undo = FunctionCall(self.addMovie, movieId, op.title, op.genre, op.description)
        operation = Operation(redo,undo)
        self.__undoController.recordOperation(operation)

    def updateMovie(self,movieId,title,genre,description):
        """
        ...
        """
        self.__undoController.newOperation()
        cop = self.__movieRepository.findById(movieId)
        movieOld = Movie(movieId,cop.title, cop.genre, cop.description)
        if title == "" and genre == "" and description == "":
            op = Movie(None,"","","")
            self.__movieRepository.update(op)
        elif title == "" and genre == "" and description != "":
            op = Movie(movieId,cop.title,cop.genre,description)
            self.__movieRepository.update(op)
        elif title == "" and genre != "" and description == "":
            op = Movie(movieId,cop.title,genre,cop.description)
            self.__movieRepository.update(op)
        elif title != "" and genre == "" and description == "":
            op = Movie(movieId,title,cop.genre,cop.description)
            self.__movieRepository.update(op)
        elif title != "" and genre != "" and description == "":
            op =Movie(movieId,title,genre,cop.description)
            self.__movieRepository.update(op)
        elif title != "" and genre == "" and description != "":
            op =Movie(movieId,title,cop.genre,description)
            self.__movieRepository.update(op)
        elif title == "" and genre != "" and description != "":
            op =Movie(movieId,cop.title,genre,description)
            self.__movieRepository.update(op)
        elif title != "" and genre != "" and description != "":
            op =Movie(movieId,title,genre,description)
            self.__movieRepository.update(op)
        redo = FunctionCall(self.updateMovie, op.entityId, op.title, op.genre, op.description)
        undo = FunctionCall(self.updateMovie, movieOld.entityId, movieOld.title, movieOld.genre, movieOld.description)
        operation = Operation(redo,undo)
        self.__undoController.recordOperation(operation)
