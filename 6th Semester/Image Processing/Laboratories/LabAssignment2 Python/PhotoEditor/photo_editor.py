...
class FiltersTab(QWidget):
    ...
    def on_filter_select(self, filter_name, e):
        ...
        if filter_name != "none":
            _img_preview = img_helper.color_filter(_img_original, filter_name, original_img=_img_preview)
        else:
        ...
        self.parent.parent.place_preview_img(filterApplyed=True)
...
class MainLayout(QVBoxLayout):
    ...
    def __init__(self, parent):
        ...
        self.img_lbl = QLabel("<h1>Welcome!</h1>"
                              "<p> In this app you are going to be able to apply filters on you photo from your vacation<p>"
                              "Available filters in this release: "
                              "<ul><li>Grayscale Filter</li>"
                              "<li>Primary Filter</li>"
                              "<li>Negative Filter</li>"
                              "<li>Red Filter</li></ul>"
                              "Press <b>'Upload'</b> to to start<br>")
        self.img_lbl.setAlignment(Qt.AlignRight)

        self.img_original = QLabel("<h1>Yeah</h1>")
        self.img_original.setAlignment(Qt.AlignLeft)

        self.file_name = None

        self.img_size_lbl = None
        self.img_size_lbl = QLabel()
        self.img_size_lbl.setAlignment(Qt.AlignLeft)
        ...
        self.addWidget(self.img_original)
    ...
    def place_preview_img(self, filterApplyed=None):
        img = _get_img_with_all_operations()

        preview_pix = ImageQt.toqpixmap(img)
        self.img_lbl.setPixmap(preview_pix)
        if not filterApplyed:
            global original_image
            original_image = preview_pix
            self.img_original.setPixmap(preview_pix)
    ...
    def update_img_size_lbl(self):
        self.img_size_lbl.setText(
            f"<span style='font-size:11px'>image size {_img_original.width} × {_img_original.height}</span>")
...
class EasyPzUI(QWidget):
    ...
    def __init__(self):
        ...
        self.setGeometry(600, 600, 600, 900)
