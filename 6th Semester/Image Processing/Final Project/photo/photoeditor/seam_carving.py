...
class Carver:
    def __init__(self, img, out_height, out_width):
        # initialize parameter
        self.out_height = out_height
        self.out_width = out_width

        # read in image and store as np.float64 format
        self.in_image = cv2.imdecode(img, cv2.IMREAD_UNCHANGED).astype(np.float64)
        ...
        # update the img to have the same state like in the begining
        self.in_image = self.out_image.astype(np.uint8)
